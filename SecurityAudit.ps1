...
Save-Section "32_network_activity" {
    ...
    $Results | Sort-Object ProcessName, RemoteClass, RemoteAddress, RemotePort
}
...
# =========================================================
# Sync SecurityAudit report to Google Drive
...
$GoogleDriveRoot = "G:\我的雲端硬碟\SecurityAudit Reports"

# Per-computer / per-date destination
$ComputerFolder = Join-Path $GoogleDriveRoot $ComputerName
$DriveDateFolder = Join-Path $ComputerFolder $Today

if ((Test-Path $GoogleDriveRoot) -and (Test-Path $OutDir)) {

    try {

        # Create computer-name folder if needed
        New-Item `
            -ItemType Directory `
            -Force `
            -Path $ComputerFolder |
        ...
        New-Item `
            -ItemType Directory `
            -Force `
            -Path $DriveDateFolder |
            Out-Null

        # Copy only today's SecurityAudit report files
        Get-ChildItem `
            -Path $OutDir `
            -File |
            Copy-Item `
                -Destination $DriveDateFolder `
                -Force `
                -ErrorAction Stop
    ...
    Write-Host ""
    Write-Host "WARNING: SecurityAudit sync failed."
    Write-Host "Computer : $ComputerName"
    Write-Host "Source   : $OutDir"
    Write-Host "Drive    : $GoogleDriveRoot"
    Write-Host ""
}
